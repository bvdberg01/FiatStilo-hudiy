...
def readLine(line, characters, client):
    ...
    if(GPIO.input(C1) == 1):
        ...
        if(characters[0] == "back"):
            key_type = api.KeyEvent.KEY_TYPE_PREVIOUS_TRACK
            print("back")
    ...
    if(GPIO.input(C2) == 1):
        ...
        if(characters[1] == "SRC"):
            key_type = api.KeyEvent.KEY_TYPE_MEDIA_MENU

        if(characters[1] == "NAV"):
            key_type = api.KeyEvent.KEY_TYPE_NAVIGATION_MENU
    ...
    if(GPIO.input(C3) == 1):
        ...
        if(characters[2] == "pause/play"):
            key_type = api.KeyEvent.KEY_TYPE_TOGGLE_PLAY
            print("play")

        if(characters[2] == "call"):
            key_type = api.KeyEvent.KEY_TYPE_ANSWER_CALL

        if(characters[2] == "Esc"):
            key_type = api.KeyEvent.KEY_TYPE_BACK
    ...
    if(GPIO.input(C4) == 1):
        ...
        if(characters[3] == "MIC"):
            key_type = api.KeyEvent.KEY_TYPE_VOICE_COMMAND
    ...
    if(GPIO.input(C5) == 1):
        ...
        if(characters[4] == "foward"):
            key_type = api.KeyEvent.KEY_TYPE_NEXT_TRACK
            print("foward")

        if(characters[4] == "TEL"):
            key_type = api.KeyEvent.KEY_TYPE_PHONE_MENU

        if(characters[4] == "Enter"):
            key_type = api.KeyEvent.KEY_TYPE_ENTER
        if(characters[4] == "12"):
            print("12")
    ...
    if(GPIO.input(C6) == 1):
        ...
        if(characters[5] == "MAIN"):
            key_type = api.KeyEvent.KEY_TYPE_HOME
    ...
    if key_type is not None:
        key_event = api.KeyEvent()
        key_event.key_type = key_type

        key_event.event_type = api.KeyEvent.EVENT_TYPE_PRESS
        client.send(api.MESSAGE_KEY_EVENT, 0,
                    key_event.SerializeToString())

        key_event.event_type = api.KeyEvent.EVENT_TYPE_RELEASE
        client.send(api.MESSAGE_KEY_EVENT, 0,
                    key_event.SerializeToString())
