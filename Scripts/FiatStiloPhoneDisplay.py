...
def splitnameorphone(input):
    for i in range(0, len(input)):
        ...
        if i <= len(input):
            char_value = dictionary[input[i].upper()]  # Neem elk karakter van de naamstring op basis van de index
        # Wijs de waarde toe aan de variabele in de dictionary
...
class EventHandler(ClientEventHandler):
    def on_hello_response(self, client, message):
        ...
        set_status_subscriptions.subscriptions.append(
            api.SetStatusSubscriptions.Subscription.PHONE)
        client.send(api.MESSAGE_SET_STATUS_SUBSCRIPTIONS, 0,
