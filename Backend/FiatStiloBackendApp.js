...
channel.addListener("onMessage", function(msg){
    // console.log("The id is:" + msg.id + " the data is " + [...msg.data])
    ...
    if(msg.id == 0x180){
        ...
        if((msg.data.readUIntBE(1,1) & 0b00001000) == 8){
            brightness(parsedData.BrightnessOn);
            currentState = true;
        }
        if((msg.data.readUIntBE(1,1) & 0b00001000) == 0){
            brightness(parsedData.BrightnessOff);
            currentState = false;
        }
    }
...
io.on('connection', function(socket){
    ...
    socket.on("brightness", (msg) => {
        if(currentState == false){
            brightness(msg.BrightnessOff)
        }

        if(currentState == true){
            brightness(msg.BrightnessOn)
        }
